...
function start_algorithm_callback(data,status) {
	...
	localStorage['quad_pdhg_task_id'] = data.task_id;
...
function poll_for_task_callback(data,status) {
	...
	var data = JSON.parse(data);
	$('#pid').text(data.pid);
	localStorage['quad_pdhg_pid'] = data.pid;
...
function init() {
	...
	$('#pid').text(localStorage['quad_pdhg_pid'] || '');
	$('#task_id').text(localStorage['quad_pdhg_task_id'] || '');
}