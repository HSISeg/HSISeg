...
function start_algorithm(request_data_json) {
	$.post("/get-weight",request_data_json,start_algorithm_callback,"text");
	return;
}
...
function start_algorithm_callback(data,status) {
	...
	localStorage['weight_task_id'] = data.task_id;
...
function poll_for_task_callback(data,status) {
	...
	var data = JSON.parse(data);
	$('#pid').text(data.pid);
	localStorage['weight_pid'] = data.pid;
	update_progress_bar(data.percentage_done,true);
	if (!data.is_done){
		setTimeout(function(){ poll_for_task(data.task_id); }, 2000);
	}else{
		$('#weight_pickle_file_path').val(data.result_file_name);
		localStorage['weight_pickle_file_path'] = data.result_file_name;
