...
def save_data_in_PUstats(values):
    ...
    c = conn.cursor()
    query = '''INSERT INTO PUstats (pos_class, neg_class, precision, recall, true_pos, true_neg, false_pos, false_neg, test_type, exclude_class_indx) VALUES (?, ?, ?, ?, ?, ?, ?, ? ,?, ?) '''
    c.executemany(query, [values])
    conn.commit()
    conn.close()

def get_visual_results(target_mat, model, input_mat, train_lp_pos_pixels, train_up_pos_pixels, train_neg_pixels, test_pos_pixels, test_neg_pixels, test_type, pos_class, neg_class_list, exclude_list ):
    ...
    save_data_in_PUstats((str(pos_class), ",".join([str(i) for i in neg_class_list]), precision, recall, int(tp), int(tn), int(fp), int(fn), test_type, ",".join([str(i) for i in exclude_list])))
    predicted_img[test_pixels] = h
    return gt_img, predicted_img, train_lp_pos_pixels, train_up_pos_pixels, train_neg_pixels, test_pos_pixels, test_neg_pixels, exclude_pixels