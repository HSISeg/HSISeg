...
def get_binary_data(pos_pixels, unlabelled_pixels, data_img):
    X = np.concatenate((data_img[pos_pixels], data_img[unlabelled_pixels]))
    Y = np.asarray(np.concatenate((np.full(len(pos_pixels[0]), 1), np.full(len(unlabelled_pixels[0]), 0))), dtype=np.int32)
    return X, Y
...
def get_PU_data(pos_class_list, data_img, labelled_img, train_pos_percentage, pos_neg_ratio_in_train, cross_pos_percentage, pos_neg_ratio_in_cross):
