...
def run():
    ...
    for pos_class in include_class_list:
        ...
        if len(neg_class_list) > 0:
            for ratio in [1, 0.82, 0.67, 0.54, 0.43, 0.33, 0.25]:
                test_name = 'type_1_cross_'+str(ratio)
                ...
                # if pos_class == 2:
                    ...
                    (train_lp_pixels, train_up_pixels, train_un_pixels, test_pos_pixels, test_neg_pixels, shuffled_test_pixels) = get_PU_data([pos_class] , neg_class_list, data_img, labelled_img, ratio, Config.type_1_cross_pos_percentage, Config.type_1_pos_neg_ratio_in_cross)
