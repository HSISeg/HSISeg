...
def run_pdhg_linear(image,cluster_number,output_path,maxconn,params,pid_element):
	...
	if pid_element:
		...
		print('Weight calculated/Retreived ', 'percentage_done ', '0')
	...
	if pid_element:
		...
		print('Centroid calculated/Retreived ', 'percentage_done ', '0')
...
def pdhg_linear(image,W,mu,endmem,lamda,tao,sigma,theta,iter_stop,innerloop,outerloop,output_path,maxconn,pid_element):
	...
	p = [sparse.csc_matrix((r,r),dtype=np.float64) for i in range(0,cluster_no)]
	count = 0
	diff = 1
	outer_index = 0
	...
	while stop < iter_stop and count < innerloop*outerloop:
		if pid_element:
			...
			print("Segmentation going on.. ", "percentage_done ", (count*100/(innerloop*outerloop)))
		outer_index += 1
		...
		f = 0.5*lamda*temp**2
		for jj in range(0,innerloop):
			count += 1
			uold = np.array(u, copy=True)
			data_inputs = [0 for i in range(0,cluster_no)]
			for l in range(0,cluster_no):
		...
		ih.save_output(L, endmem, output_path + "_data_" + str(outer_index) + ".pickle")
		ih.save_image(L,output_path + "_image_" + str(outer_index) + ".jpeg",colors)
