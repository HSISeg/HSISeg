...
from semiSuper.DistanceBasedSampling import get_PU_data as get_PU_data_dist
import utils
import copy
import Config
import datetime
        ...
        if len(neg_class_list) > 0:
            for ratio in Config.type_1_pos_neg_ratio_in_train:
                ...
                if not utils.check_if_test_done_models(str(pos_class), test_name, ",".join([str(i) for i in neg_class_list]), Config.data, ratio, False):
                    # print(len(clust_labelled_img), len(clust_labelled_img[0]),"clust_labelled_img")
...
def run_cluster(clust_labelled_img, data_img, labelled_img):
    # cross validation ratio change
    include_class_list = Config.type_1_include_class_list
    for pos_class in include_class_list:
        neg_class_list = copy.copy(list(set(include_class_list)))
        ...
        if len(neg_class_list) > 0:
            for ratio in Config.type_1_pos_neg_ratio_in_train:
                ...
                if not utils.check_if_test_done_models(str(pos_class), test_name, ",".join([str(i) for i in neg_class_list]), Config.data, ratio, False):
                    ...
                    exclude_list = list(set([i for i in range(n_class)]) - set(include_class_list))
                    (XYtrain, XYtest, prior, testX, testY, trainX, trainY, crossX, crossY), \
                    (train_lp_pixels, train_up_pixels, train_un_pixels, test_pos_pixels, test_neg_pixels, shuffled_test_pixels) = get_PU_data([pos_class], neg_class_list, data_img, labelled_img, clust_labelled_img, Config.type_1_train_pos_percentage, ratio, Config.type_1_cross_pos_percentage, ratio, False)
                    print("training", trainX.shape)
                    print("training split: labelled positive ->", len(train_lp_pixels[0]), "unlabelled positive ->", len(train_up_pixels[0]), "unlabelled negative ->", len(train_un_pixels[0]))
                    print("test", testX.shape)
                    model = train_PN(trainX, trainY, testX, testY)
...
def run_dist(clust_labelled_img, data_img, labelled_img):
    # cross validation ratio change
    include_class_list = Config.type_1_include_class_list
    for pos_class in include_class_list:
        neg_class_list = copy.copy(list(set(include_class_list)))
        ...
        if len(neg_class_list) > 0:
            for ratio in Config.type_1_pos_neg_ratio_in_train:
                ...
                if not utils.check_if_test_done_models(str(pos_class), test_name, ",".join([str(i) for i in neg_class_list]), Config.data, ratio, False):
                    ...
                    exclude_list = list(set([i for i in range(n_class)]) - set(include_class_list))
                    (XYtrain, XYtest, prior, testX, testY, trainX, trainY, crossX, crossY), \
                    (train_lp_pixels, train_up_pixels, train_un_pixels, test_pos_pixels, test_neg_pixels, shuffled_test_pixels) = get_PU_data_dist([pos_class], neg_class_list, data_img, labelled_img, None, Config.type_1_train_pos_percentage, ratio, Config.type_1_cross_pos_percentage, ratio, True)
                    print("training", trainX.shape)
                    print("training split: labelled positive ->", len(train_lp_pixels[0]), "unlabelled positive ->", len(train_up_pixels[0]), "unlabelled negative ->", len(train_un_pixels[0]))
                    print("test", testX.shape)
                    model = train_PN(trainX, trainY, testX, testY)
