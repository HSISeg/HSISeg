...
class TanhCrossEntropy(function_node.FunctionNode):

    """Sigmoid activation followed by a sigmoid cross entropy loss."""

    ignore_label = -1

    def __init__(self, normalize=False, reduce='mean'):
    ...
    def backward(self, inputs, grad_outputs):
        ...
        return TanhCrossEntropyGrad(
            self.reduce, self.count, self.ignore_mask, t.data).apply((x, gy))


class TanhCrossEntropyGrad(function_node.FunctionNode):
...
def tanh_cross_entropy(x, t, normalize=False, reduce='mean'):
    ...
    """
    return TanhCrossEntropy(normalize, reduce).apply((x, t))[0]
