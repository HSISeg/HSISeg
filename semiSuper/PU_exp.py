...
# with guess
def run_human_guess(clust_labelled_img, data_img, labelled_img):
                    ...
                    for base in Config.baseline_test_list:
                        for temp in Config.temperature_test_list:
                            if not utils.check_if_test_done_models(str(pos_class), test_name, ",".join([str(i) for i in neg_class_list]), Config.data, ratio, True, k, base, temp):
                                ...
                                (train_lp_pixels, train_up_pixels, train_un_pixels, test_pos_pixels, test_neg_pixels, shuffled_test_pixels) = get_PU_data([pos_class], neg_class_list, 
                                    data_img, labelled_img, clust_labelled_img, Config.type_1_train_pos_percentage, 
                                    ratio, Config.type_1_cross_pos_percentage, True, base, temp, calculate_prior=False)
                                print("training", trainX.shape, pos_class)
                                ...
                                test_loss_output_path = "test_" + str(test_name) + str(datetime.datetime.now().timestamp() * 1000) + ".png"
                                train_loss_output_path = "train_" + str(test_name) + str(datetime.datetime.now().timestamp() * 1000) + ".png"
                                model = train_PU(XYtrain, XYtest, prior, test_loss_output_path, train_loss_output_path, "result/")
                              ...
                              # generate predicted and groundtooth image
                                ...
                                PUstats.objects.create(pos_class=str(pos_class), neg_class=",".join([str(i) for i in neg_class_list]),
                                                     ...
                                                     train_neg_pos_ratio=ratio, threshold=model.threshold, auc=model.auc,
                ...
                for base in Config.baseline_test_list:
                    for temp in Config.temperature_test_list:
                        if not utils.check_if_test_done_models(str(pos_class), test_name, ",".join([str(i) for i in neg_class_list]), Config.data, None, True, k, base, temp):
                            ...
                            (train_lp_pixels, train_up_pixels, train_un_pixels, test_pos_pixels, test_neg_pixels, shuffled_test_pixels) = get_PU_data([pos_class], neg_class_list, 
                                data_img, labelled_img, clust_labelled_img, Config.type_1_train_pos_percentage, 
                                None, Config.type_1_cross_pos_percentage, True, base, temp, calculate_prior=True)
                          ...
                          # generate predicted and groundtooth image
                            ...
                            PUstats.objects.create(pos_class=str(pos_class), neg_class=",".join([str(i) for i in neg_class_list]),
                                                 ...
                                                 no_train_pos_unlabelled=int(len(train_up_pixels[0])),
                                                 no_train_neg_unlabelled=int(len(train_un_pixels[0])),
                                                 train_neg_pos_ratio=None, threshold=model.threshold, auc=model.auc,
                                                 data_name=Config.data,visual_result_filename=visual_result_filename,
                                                 temperature = temp, baseline= base, experiment_number=k,
                                                 experiment_time=end_time - start_time, convergence_plot_path="result/" + train_loss_output_path,
                                                 unlabelled_prior = prior)
