...
# I_p and I_c/I_c
def get_cluster_pos_prob(clust_labelled_img, train_lp_pixels, cross_pos_pixels):
    pos_sel = np.zeros(clust_labelled_img.shape, dtype=np.bool)
    pos_sel[train_lp_pixels] = True
    pos_sel[cross_pos_pixels] = True
    n_pos = len(train_lp_pixels[0]) + len(cross_pos_pixels[0])
    ...
    clust_pos_prob = np.zeros(n_classes, dtype=np.float32)
    for i in range(n_classes):
        cluster_size = len(np.where(clust_labelled_img == i)[0])
        clust_pos_prob[i] = ((np.logical_and(clust_labelled_img == i, pos_sel)).sum() + Config.epsilon) / cluster_size
    return clust_pos_prob
...
def get_point_wise_prob(clust_labelled_img, train_lp_pixels, cross_pos_pixels, is_dist_based, baseline, temp, is_uniform_sampling=False):
    ...
    n_classes = np.max(clust_labelled_img) + 1
    if Config.spatial_spectral_single_term:
        # P(X_p|X_c) cluster part
        cluster_pos_prob = get_cluster_pos_prob(clust_labelled_img, train_lp_pixels, cross_pos_pixels)
        for i in range(n_classes):
            class_pixels = clust_labelled_img == i
            # bayes rule
            final_prob[class_pixels] = dist[class_pixels] * cluster_pos_prob[i]
    else:
        # P(cluster|X)
        ...
        for i in range(n_classes):
            class_pixels = clust_labelled_img == i
            # bayes rule
            final_prob[class_pixels] = (dist[class_pixels] * clust_pos_prob[i])
    # final_prob = final_prob / np.sum(final_prob)
    final_prob = 1 - final_prob
    return final_prob

def connected_positive_sampling(n_train_pos_pixels, n_cross_pos_pixels, gt_labelled_img, pos_pixels, pos_class_list):
...
def get_pos_pixels(pos_class_list, gt_labelled_img, train_pos_percentage, cross_pos_percentage):
    ...
    if is_random:
        ...
        train_lp_pixels, cross_pos_pixels = connected_positive_sampling(n_train_pos_pixels, n_cross_pos_pixels, gt_labelled_img, pos_pixels, pos_class_list)
    return train_lp_pixels, cross_pos_pixels
