    ...
    for pos_class in include_class_list:
        ...
        if len(neg_class_list) > 0:
            ...
            if not utils.check_if_test_done(pos_class, test_name, ",".join([str(i) for i in neg_class_list])):
                ...
                (train_lp_pixels, train_up_pixels, train_un_pixels, test_pos_pixels, test_neg_pixels, shuffled_test_pixels) = get_PU_data([pos_class] , data_img, labelled_img, clust_labelled_img, Config.type_1_train_pos_percentage, ratio, Config.type_1_cross_pos_percentage, ratio)
